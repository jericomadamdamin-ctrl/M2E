  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      if (seasonData) {
        ...
        if (entries) {
          const formattedEntries: LeaderboardEntry[] = entries.map((entry: SupabaseLeaderboardEntry, index: number) => ({
            rank: index + 1,
      ...
      <div className="space-y-2">
        {leaderboard.length === 0 ? (
          ...
          leaderboard.map((entry) => (
            <div
              key={entry.user_id}
              className={`card-game rounded-xl p-3 flex items-center gap-3 ${entry.user_id === currentUserId ? 'border-primary/50 glow-diamond' : ''
                } ${entry.rank <= 3 ? 'bg-gradient-to-r from-secondary/50 to-transparent' : ''}`}
            >
              <div className="w-8 flex justify-center">{getRankIcon(entry.rank)}</div>
              <div className="flex-1 min-w-0">
                <p className={`font-bold text-sm truncate ${entry.user_id === currentUserId ? 'text-primary' : ''
                  }`}>
                  {entry.player_name}
